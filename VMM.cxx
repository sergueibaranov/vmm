...
void beginColibration(std::string inputPath, std::string outputPath) {
    ...
    query = string ("/usr/bin/mkdir -p ") + outputPath + "/root";
    gSystem->Exec(query.c_str());
    
    query = string ("/usr/bin/mkdir -p ") + outputPath + "/constants";
    gSystem->Exec(query.c_str());
    
    query = string ("/usr/bin/mkdir -p ") + outputPath + "/pdf";
    gSystem->Exec(query.c_str());
    
    query = string ("/usr/bin/mkdir -p ") + outputPath + "/localDB";
    gSystem->Exec(query.c_str());
...
//        else new_mod_s = fileName;
        ...
        cout << "\n--> beginColibration(): " << new_mod_s.c_str() << "\n";
...
//            gCnv->Print(pdfopenMain, "pdf");
            ...
            cout << "\n--> beginColibration(): new_mod_s.length()= " << new_mod_s.length() << "\n";
...
#endif
{   /* 
     baranov@[aiatlas066]~/public/TGU/Software/srosliak/soft.241018> 
       time vmm_root_compile.sh VMM.cxx 30092018 18102018 & 
       time vmm_root_compile.sh VMM.cxx $(ls /rawdata) &
    */
/*
#ifndef __CINT__
...
*/
    ...
    for (int i = 2; i < argc; i++) {
      ...
      std:string codeday = (string)argv[i];
      std::string inputPath = "/rawdata/";
      std::string outputPath = "/output/";
      inputPath.append(codeday);
      outputPath.append(codeday);
    
      pathInputFile.push_back(inputPath);
      pathOutputFile.push_back(outputPath);
      cout<<"---> Main() "<< codeday << " " << inputPath << " " << outputPath <<" pathInputFile.size()= "<< pathInputFile.size() <<"\n";
    ...
    for (int i = 0; i < pathInputFile.size(); i++) {
        ...
        // remove if exest
        query = std::string ("/usr/bin/rm -rf ") + outputPath;
        gSystem->Exec(query.c_str());

        // make again
        query = string ("/usr/bin/mkdir -p ") + outputPath;
        gSystem->Exec(query.c_str());

        beginColibration(inputPath, outputPath);
    }
